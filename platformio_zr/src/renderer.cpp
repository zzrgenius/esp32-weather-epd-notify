...
GxEPD2_BW<GxEPD2_750, GxEPD2_750::HEIGHT> display(
    GxEPD2_750(PIN_EPD_CS, PIN_EPD_DC, PIN_EPD_RST, PIN_EPD_BUSY));
#endif
...
void drawCurrentConditions(const owm_current_t &current,
  ...
  // display.drawInvertedBitmap(0, 0,                             getCurrentConditionsBitmap196(current, today), 196,                             196, GxEPD_BLACK);
                             ...
                             getCurrentConditionsBitmap196Qweather(current, today), 196,
                             196, GxEPD_BLACK);
  ...
  display.drawInvertedBitmap(0, 204 + (48 + 8) * 0, wi_sunrise_48x48, 48, 48,
                             GxEPD_BLACK);
  display.drawInvertedBitmap(0, 204 + (48 + 8) * 1, wi_strong_wind_48x48, 48,
                             48, GxEPD_BLACK);
  display.drawInvertedBitmap(0, 204 + (48 + 8) * 2, wi_day_sunny_48x48, 48, 48,
                             GxEPD_BLACK);
#ifndef DISP_BW_V1
  display.drawInvertedBitmap(0, 204 + (48 + 8) * 3, air_filter_48x48, 48, 48,
                             GxEPD_BLACK);
  display.drawInvertedBitmap(0, 204 + (48 + 8) * 4, house_thermometer_48x48, 48,
                             48, GxEPD_BLACK);
#endif
  display.drawInvertedBitmap(170, 204 + (48 + 8) * 0, wi_sunset_48x48, 48, 48,
                             GxEPD_BLACK);
  display.drawInvertedBitmap(170, 204 + (48 + 8) * 1, wi_humidity_48x48, 48, 48,
                             GxEPD_BLACK);
  display.drawInvertedBitmap(170, 204 + (48 + 8) * 2, wi_barometer_48x48, 48,
                             48, GxEPD_BLACK);
#ifndef DISP_BW_V1
  display.drawInvertedBitmap(170, 204 + (48 + 8) * 3, visibility_icon_48x48, 48,
                             48, GxEPD_BLACK);
  display.drawInvertedBitmap(170, 204 + (48 + 8) * 4, house_humidity_48x48, 48,
                             48, GxEPD_BLACK);
#endif
  ...
  display.drawInvertedBitmap(48, 204 + 24 / 2 + (48 + 8) * 1,
                             getWindBitmap24(current.wind_deg), 24, 24,
                             GxEPD_BLACK);
#endif
  ...
  if (getStringWidth(dataStr) <=
    ...
    if (getStringWidth(dataStr) <=
        max_w) {  // Fits on a single line with smaller font, draw along bottom
      drawString(display.getCursorX() + sp,
                 204 + 17 / 2 + (48 + 8) * 3 + 48 / 2, dataStr, LEFT);
    } else {  // Does not fit on a single line, draw higher to allow room for
              // 2nd line
      drawMultiLnString(display.getCursorX() + sp,
                        204 + 17 / 2 + (48 + 8) * 3 + 48 / 2 - 10, dataStr,
                        LEFT, max_w, 2, 10);
    }
  }
  ...
  dataStr = String(
      static_cast<int>(std::round(hectopascals_to_pascals(current.pressure))));
  unitStr = TXT_UNITS_PRES_PASCALS;
#endif
#ifdef UNITS_PRES_MILLIMETERSOFMERCURY
  dataStr = String(static_cast<int>(
      std::round(hectopascals_to_millimetersofmercury(current.pressure))));
  ...
  dataStr = String(
      std::round(1e3f * hectopascals_to_atmospheres(current.pressure)) / 1e3f,
      3);
  unitStr = TXT_UNITS_PRES_ATMOSPHERES;
#endif
#ifdef UNITS_PRES_GRAMSPERSQUARECENTIMETER
  dataStr = String(static_cast<int>(
      std::round(hectopascals_to_gramspersquarecentimeter(current.pressure))));
  unitStr = TXT_UNITS_PRES_GRAMSPERSQUARECENTIMETER;
#endif
#ifdef UNITS_PRES_POUNDSPERSQUAREINCH
  dataStr = String(
      std::round(1e2f * hectopascals_to_poundspersquareinch(current.pressure)) /
          1e2f,
      2);
  ...
  void drawForecast(owm_daily_t *const daily, tm timeInfo) {
    ...
    int x = 318 + (i * 64);
      ...
      //                            64, 64, GxEPD_BLACK);
      display.drawInvertedBitmap(x, 98 + 69 / 2 - 32 - 6,
                                 getForecastBitmap64Qweather(daily[i]), 64, 64,
                                 GxEPD_BLACK);
...
// daily forecast precipitation
  ...
  void drawAlerts(std::vector<owm_alerts_t> & alerts, const String &city,
    ...
    if (num_valid_alerts == 1) {  // 1 alert
      ...
      if (getStringWidth(cur_alert.event) <=
        ...
        } else {  // Does not fit on a single line, draw higher to allow room
                  // for 2nd line
   ...
   * information in the top right corner.
   */
  void drawLove(const String &love) {
    // location, date
    display.setFont(&FONT_16pt8b);
    drawString(2, 23, love, LEFT, ACCENT_COLOR);
    // display.setFont(&FONT_12pt8b);
    // drawString(DISP_WIDTH - 2, 30 + 4 + 17, date, RIGHT);
    return;
  }  // end drawLocationDate
  ...
  /* This function is responsible for drawing the outlook graph for the
   * specified number of hours(up to 47).
   */
  void drawOutlookGraph(owm_hourly_t *const hourly, tm timeInfo) {
    const int xPos0 = 350;
    int xPos1 =
        DISP_WIDTH - 23;  // may be moved to make room for decimal places
    const int yPos0 = 216;
  ...
  float precipRoundingMultiplier = std::pow(10.f, yPrecipMajorTickDecimals);
    ...
    int xMaxTicks = 8;
    int hourInterval = static_cast<int>(
        ceil(HOURLY_GRAPH_MAX / static_cast<float>(xMaxTicks)));
    float xInterval =
        (xPos1 - xPos0 - 1) / static_cast<float>(HOURLY_GRAPH_MAX);
    display.setFont(&FONT_8pt8b);
    for (int i = 0; i < HOURLY_GRAPH_MAX; ++i) {
      ...
      if (i > 0) {
        // temperature
        x0_t = static_cast<int>(
            std::round(xPos0 + ((i - 1) * xInterval) + (0.5 * xInterval)));
        x1_t = static_cast<int>(
        ...
        y0_t = static_cast<int>(std::round(
            yPos1 - (yPxPerUnit *
                     (kelvin_to_celsius(hourly[i - 1].temp) - tempBoundMin))));
        y1_t = static_cast<int>(std::round(
            yPos1 -
            (yPxPerUnit * (kelvin_to_celsius(hourly[i].temp) - tempBoundMin))));
#endif
#ifdef UNITS_TEMP_FAHRENHEIT
        y0_t = static_cast<int>(std::round(
        ...
        y1_t = static_cast<int>(std::round(
            yPos1 - (yPxPerUnit *
                     (kelvin_to_fahrenheit(hourly[i].temp) - tempBoundMin))));
#endif
    ...
    precipVal = millimeters_to_inches(precipVal);
      ...
      if ((i % hourInterval) == 0) {
        ...
        display.drawLine(xTick + 1, yPos1 + 1, xTick + 1, yPos1 + 4,
                         GxEPD_BLACK);
        // draw x axis labels
        char timeBuffer[12] = {};  // big enough to accommodate "hh:mm:ss am"
  ...
  void drawStatusBar(const String &statusStr, const String &refreshTimeStr,
    ...
    // battery
    uint32_t batPercent = calcBatPercent(batVoltage, CRIT_LOW_BATTERY_VOLTAGE,
                                         MAX_BATTERY_VOLTAGE);
