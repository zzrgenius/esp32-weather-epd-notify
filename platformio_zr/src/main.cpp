 ...
 */
// https://devapi.qweather.com/v7/weather/now?location=101190109&lang=en&key=c1ff043362be465b8f56486df917a35c
// https://devapi.qweather.com/v7/weather/7d?location=101190109&lang=en&key=c1ff043362be465b8f56486df917a35c
// https://devapi.qweather.com/v7/weather/24h?location=101190109&lang=en&key=c1ff043362be465b8f56486df917a35c
// https://devapi.qweather.com/v7/air/now?location=101190109&lang=en&key=c1ff043362be465b8f56486df917a35c
#include <Arduino.h>
...
#endif
 int rxStatus = getWeatherNowcall(client, owm_onecall);
  if (rxStatus != HTTP_CODE_OK)
  {
    killWiFi();
    statusStr = "now " + OWM_ONECALL_VERSION + " API";
  ...
  if (rxStatus != HTTP_CODE_OK)
  {
    ...
    rxStatus = getWeatherHourcall(client, owm_onecall);
  if (rxStatus != HTTP_CODE_OK)
  {
    killWiFi();
    statusStr = "HOUR CALL " + OWM_ONECALL_VERSION + " API";
    tmpStr = String(rxStatus, DEC) + ": " + getHttpResponsePhrase(rxStatus);
    initDisplay();
    do
    {
  ...
  #if 1
  rxStatus = getQweatherairpollution(client, owm_air_pollution);
  if (rxStatus != HTTP_CODE_OK)
  {
    killWiFi();
  ...
  Adafruit_BME280 bme;    // Adafruit_BMP280 bme(&I2C_bme);
  ...
  Serial.print( " temp: "+String(inTemp)+" humidity: " +String(inHumidity)+" ");
